# achah 分佈式 LLM 推理桌面應用程式

一個分佈式 LLM 推理程式，讓您能夠利用區域網路內的多台電腦協同進行大型語言模型的分佈式推理。
将您现有的设备统一到一个强大的 GPU 中：iPhone、iPad、Android、Mac、Windows、Raspberry Pi，几乎任何设备！
...
- 🚀 **跨平台**: 支援 Windows、macOS、iPhone、Android和 Linux，統一體驗
...
- Python 12+ 
- 支援的作業系統：Windows 11+、macOS 10.15+、Ubuntu 18.04+
- 至少 8GB RAM（取決於模型大小）
...
netstat -an | grep :5052

# 測試網路連通性
ping 192.168.1.101
telnet 192.168.1.101 5052
...
3. **防火牆**: 嚴格限制 5052 和 8080 端口的存取來源
4. **監控**: 持續監控網路流量和節點狀態
